...
async function generateRooms() {
  for (let i = 0; i < 15; i++) {
    ...
    await db.promise().query(`INSERT INTO \`rooms\` (name, bed_type, photo, description, amenities, rate, offer, available) 
      VALUES (?, ?, ?, ?, ?, ?)`,
    [o.name, o.bed_type, o.photo, o.description, o.amenities, o.rate, o.offer, o.available]);
  }
}
...
async function generateBookings() {
  ...
  for (let i = 0; i < 15; i++) {
    ...
    await db.promise().query(`INSERT INTO \`rooms\` (room_id, guest, guest_id, photo, order_date, check_in, check_out, room_type, special_request) 
...
async function generateContacts() {
  for (let i = 0; i < 15; i++) {
    ...
    await db.promise().query(`INSERT INTO \`rooms\` (date, name, email, phone, subject, comment, archived) 
...
async function generateUsers() {
  for (let i = 0; i < 15; i++) {
    ...
    await db.promise().query(`INSERT INTO \`rooms\` (full_name, description, email, password, photo, position, active, username, phone, start_date) 
      VALUES (${o.full_name}, ${o.description}, ${o.email}, ${o.password}, ${o.photo}, ${o.position}, ${o.active}, ${o.username}. ${o.phone}, ${o.start_date})`);
  }
}
